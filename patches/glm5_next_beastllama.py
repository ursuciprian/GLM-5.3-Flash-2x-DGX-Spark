...
class Glm5NextModel(nn.Module):
    ...
    def _prepare_aux_hidden_state(
        ...
        # GB10 day-0 fix: during CUDA graph capture residual is None; the
        # hidden state is already the full stream in that case (same guard as
        # the EAGLE-3 aux capture path in llama.py).
        if residual is None:
